"""
Training script for RMSNorm ablation.
Uses TransformerNoRMSNorm instead of standard Transformer.
"""
import argparse
import numpy as np
import os
...
from cs336_basics.ablation_no_rmsnorm import (
    TransformerNoRMSNorm,
    AdamW,
    save_checkpoint,
    load_checkpoint,
    get_batch,
...
def parse_args():
    parser = argparse.ArgumentParser(description="CS336 RMSNorm Ablation Training")
...
if __name__ == "__main__":
    args = parse_args()
    print(f"=== RMSNorm Ablation: lr={args.lr_max} ===")

    os.makedirs(args.ckpt_save_path, exist_ok=True)
    run_name = args.wandb_name if args.wandb_name else f"no_rmsnorm_lr_{args.lr_max}"
    ...
    print(f"Model params: {total_params:,} (no RMSNorm)")
