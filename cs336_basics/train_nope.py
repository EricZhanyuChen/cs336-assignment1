"""
Training script for NoPE (No Positional Embedding) ablation.
Uses TransformerNoPE instead of standard Transformer with RoPE.
"""
import argparse
import numpy as np
import os
...
from cs336_basics.ablation_nope import (
    TransformerNoPE,
    AdamW,
    save_checkpoint,
    load_checkpoint,
...
def parse_args():
    parser = argparse.ArgumentParser(description="CS336 NoPE Ablation Training")
...
if __name__ == "__main__":
    args = parse_args()
    print(f"=== NoPE Ablation: lr={args.lr_max} ===")

    os.makedirs(args.ckpt_save_path, exist_ok=True)
    run_name = args.wandb_name if args.wandb_name else f"nope_lr_{args.lr_max}"
