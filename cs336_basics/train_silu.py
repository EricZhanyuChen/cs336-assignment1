"""
Training script for SiLU vs SwiGLU ablation.
Replaces SwiGLU FFN with standard SiLU (no gate w3).
"""
import argparse
import numpy as np
import os
...
from cs336_basics.ablation_silu import (
    TransformerWithSiLU,
    AdamW,
    save_checkpoint,
    load_checkpoint,
...
def parse_args():
    parser = argparse.ArgumentParser(description="CS336 SiLU Ablation Training")
...
if __name__ == "__main__":
    args = parse_args()
    print(f"=== SiLU Ablation: lr={args.lr_max} ===")

    os.makedirs(args.ckpt_save_path, exist_ok=True)
    run_name = args.wandb_name if args.wandb_name else f"silu_lr_{args.lr_max}"
    ...
    print(f"Model params: {total_params:,} (no RoPE)")
